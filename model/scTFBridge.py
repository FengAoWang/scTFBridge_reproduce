...
def explain_TF2TG(scModel: scTFBridge,
    ...
    if cell_type != 'cellular':
        print("Calculating TF2TG regulatory value for: ", cell_type)
    ...
    print(f'{cell_type} finish computation')
...
def explain_RE2TG(scModel: scTFBridge,
    ...
    test_all_data, test_rna_data = get_sample_data(data_loader, num_samples=50)

    all_attributions = []

    #   compute RE shap value
    gene_ids = []
    common_geneSymbol = []
    ...
    shap_all = pd.DataFrame(shap_values, columns=common_geneSymbol, index=atac_peak_feature)
